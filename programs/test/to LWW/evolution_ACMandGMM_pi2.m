function [Pros, phi] = evolution_ACMandGMM_pi2( Pros, image_original ,image_processed)
% evolution_ACMandGMM 基于文献[1]的演化模型的改造。用的先验信息是式(25）。
%   input:
% Pros：Pros结构体
...
while (Pros.iteration_outer<=Pros.numIteration_outer && numPixelChanged>=Pros.numPixelChangedToContinue)
	...
	phi = reshape(phi, [],1);
	Pi_vis(:,1)=1./(1+exp(Pros.gamma-Pros.beta.*curvature-phi./Pros.epsilon));
	Pi_vis(:,2)=1-Pi_vis(:,1);
	Prior(1)=mean(Pi_vis(:,1));
	Prior(2)=1-Prior(1);
