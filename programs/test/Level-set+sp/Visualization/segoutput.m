function [segOutline, imgMarkup,RGBImgMarkup]=segoutput( img,RGBImg, solution,  arguments, properties)
...
%% Visualization
if strcmp(arguments.isVisualSegoutput ,'yes')==1
    properties.handles_figure_segoutput = figure('name','超像素可视化');
    subplot(3,1,1);
    imshow(segOutline);
    title('超像素边界');
    for i=1:1:properties.numSP
        text(properties.SuperPixelsPos(i,2),properties.SuperPixelsPos(i,1), num2str(i),'FontSize',8); % 显示标号
    end
    subplot(3,1,2);
    imshow(imgMarkup);
    title([arguments.colorspace,' 图像中的超像素分布']);
    subplot(3,1,3);
    imshow(RGBImgMarkup);
    ...
    %% save screen shot
    properties.folderpath_visualize_segoutput = ([properties.folderpath,'\','screen shot','\','segment output']);
    sentence015=(['mkdir(''',properties.folderpath_visualize_segoutput,''');']);
    eval(sentence015);
    folderpath020=([properties.folderpath_visualize_segoutput,'\','segment output']);
