...
% 实验3：引入超像素力的超像素化了的半监督ACMGMM的实验系列
...
Args.folderpath_ResultsBaseFolder = '.\data\ACMGMMSEMI\MSRA1K 10 images\segmentations\试验有无SP的区别';
...
Args.evolutionMethod='semiACMGMMSP_2';
Args=initSegmentationMethod(Args, Args.evolutionMethod);

%% parameters settings
% probability term
Args.timestep=0.3 ;  % time step, default = 0.10
Args.epsilon = 0.10 ; % papramater that specifies the width of the DiracDelta function. default=0.05 and more at ref [1].
Args.beta =1.0 ; % coefficeient of the weighted length term L(phi). default=0.5. also 0.001*255*255.
Args.gamma = 0.5 ;  % coefficient of the weighted area term A(phi). default=0.2.
...
Args.scaleOflabeledProbabilities = 3.0;  % 图像的有标记样本的概率值比未标记样本的概率值的最大值（最小值）还要大（小）的比例值，默认扩缩 2.0 倍。
