function [Pros] = writeData_ACMandGMMtoEq18(Pros, image_original, image_processed, phi,  bwData, Pix, Px, Pxi, P_x_and_i, Pi, mu, Sigma)
%writeData_ACMandGMMtoEq18 保存在程序运行过程中产生的数据，并不是每一代都保存，而是保存给定迭代周期的数据
%   input:
% Pros：结构体
...
% Px：高斯概率密度。图像像素数×1
% Pxi：条件概率。图像像素数×分类数
% P_x_and_i：同时满足x和i的概率。图像像素数×分类数
% Pi：先验概率。1×分类数
% mu：均值。维数×维数
% Sigma：协方差。维数×维数×分类数
% output:
