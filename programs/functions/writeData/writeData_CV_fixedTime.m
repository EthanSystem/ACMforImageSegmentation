function [Pros] = writeData_CV_fixedTime(Pros, image_original, image_processed, phi,  bwData, areaTerm, regularTerm, edgeTerm, diracPhi, DeltaPhi, Pi, mu, Sigma)
%writeData_CV_fixedTime 保存在程序运行过程中产生的数据，并不是每一代都保存，而是保存给定迭代周期的数据
%   input:
%% TODO
% Pros：结构体
...
% 	ExportData.data(i).Pix = reshape(areaTerm,numRow,numCol,numState);
% 	ExportData.data(i).Px = reshape(regularTerm,numRow,numCol);
% 	ExportData.data(i).Pxi = reshape(edgeTerm,numRow,numCol,numState);
% 	ExportData.data(i).P_x_and_i = reshape(diracPhi,numRow,numCol,numState);
% 	ExportData.data(i).Pi_vis = reshape(DeltaPhi,numRow,numCol,numState);
