function [ Results ] = createResultsStructure( baseFolder, numUselessFiles )
...
if Results.num_experiments~=0
	for index_experiment=1:Results.num_experiments
		%% 指定实验的文件夹
		Results.experiments(index_experiment).folderpath=fullfile(Results.folderpath_outputBase, Results.experiments(index_experiment).name);
% 		foldersInEachExperiments=dir(Results.experiments(index_experiment).folderpath);
		
		%% 生成diary输出文件夹
		Results.experiments(index_experiment).diary.name = 'diary output';
		Results.experiments(index_experiment).diary.folderpath = fullfile(Results.experiments(index_experiment).folderpath, Results.experiments(index_experiment).diary.name);
		...
		%% 最终分割二值图截图文件夹路径
		Results.experiments(index_experiment).folderpath_bwImage = fullfile(Results.experiments(index_experiment).folderpath, 'bw images');
		if ~exist(Results.experiments(index_experiment).folderpath_bwImage,'dir')
			mkdir(Results.experiments(index_experiment).folderpath_bwImage);
		end
		% 每个最终分割二值图截图名称和路径
		...
		disp(['实验文件夹 ' Results.experiments(index_experiment).name ' 的文件夹 bw images 有 ' num2str(Results.experiments(index_experiment).num_bwImage) ' 个文件'])
		...
		%% 最终分割二值数据文件夹路径
		Results.experiments(index_experiment).folderpath_bwData = fullfile(Results.experiments(index_experiment).folderpath, 'bw data');
		...
		%% 最终嵌入函数文件夹路径
		Results.experiments(index_experiment).folderpath_phiData = fullfile(Results.experiments(index_experiment).folderpath, 'phi data');
		...
		%% 标记图像的标记种子数据文件夹路径
		Results.experiments(index_experiment).folderpath_seeds = fullfile(Results.experiments(index_experiment).folderpath, 'seeds data');
		...
		%% 结果截图文件夹路径
		Results.experiments(index_experiment).folderpath_screenShot = fullfile(Results.experiments(index_experiment).folderpath, 'screen shot');
		...
		%% 输出数据文件夹路径
		Results.experiments(index_experiment).folderpath_writeData = fullfile(Results.experiments(index_experiment).folderpath, 'write data');
		...
		%% 输出评价值数据的文件夹路径
		Results.experiments(index_experiment).folderpath_evaluationData = fullfile(Results.experiments(index_experiment).folderpath, 'evaluation data');
